        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <label>
            <span style={{ fontWeight: 500 }}>
              Короткий заголовок (ShortTitle)
            </span>
            <input
              type="text"
              value={question.shortTitle || ""}
              onChange={(e) =>
                onChange({ ...question, shortTitle: e.target.value })
              }
              style={{ width: "100%", marginTop: 4 }}
            />
          </label>
        ...
        <QuestionRenderer
          ...
          onOptionChange={(optIdx, field, value) => {
            ...
            if (field === "text") {
              ...
              const scores = newOptions[optIdx].scores
                ? [...newOptions[optIdx].scores]
                : [];
              const selectedScoreType = scoreTypes.find(
                (st) => st.code === value,
              );
              if (scores[0]) {
                scores[0] = {
            ...
            onChange({ ...question, options: newOptions } as
              | SingleChoiceQuestion
              | MultipleChoiceQuestion);
          }}
          onOptionAdd={() => {
