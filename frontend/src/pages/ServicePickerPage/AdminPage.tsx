    ...
    <div className={styles.container}>
      <div className={styles.mainContent}>
        <div className={styles.questionsContainer}>
          {adminQuestions.filter(q => q.isRequired === false).map((question, idx) => (
            <div key={question.id} id={`question-${question.id}`} style={{ display: 'flex', alignItems: 'center', marginBottom: 16 }}>
              <div style={{ flex: 1 }} className={!question.active ? styles.inactiveQuestion : undefined}>
                <QuestionRenderer
                  question={question}
                  isAdmin={true}
                  ...
                  onQuestionTextChange={newText => {
                    const realIdx = adminQuestions.findIndex(q => q.id === question.id);
                    if (realIdx === -1) return;
                    const newQuestions = [...adminQuestions];
                    newQuestions[realIdx] = { ...question, questionText: newText };
                  ...
                  onOptionChange={(optIdx, field, value) => {
                    ...
                    if (field === 'text') {
                      ...
                      const selectedType = scoreTypes.find(st => st.code === value);
                      const newScores = newOptions[optIdx].scores ? [...newOptions[optIdx].scores] : [{ id: newOptions[optIdx].id, code: '', title: '', weight: 0 }];
                      newScores[0] = {
                        ...newScores[0],
                      ...
                      const newScores = newOptions[optIdx].scores ? [...newOptions[optIdx].scores] : [{ id: newOptions[optIdx].id, code: '', title: '', weight: 0 }];
                      newScores[0] = { ...newScores[0], weight: value === '' ? 0 : Number(value) };
                      newOptions[optIdx] = { ...newOptions[optIdx], scores: newScores };
                    }
                    newQuestions[realIdx] = { ...question, options: newOptions };
                    setAdminQuestions(newQuestions);
                  }}
                  onOptionAdd={() => {
                    if (question.type !== 'single-choice' && question.type !== 'multiple-choice') return;
                    ...
                    const newOptions = [
                      ...(question.options || []),
                      { id: Date.now(), text: '', scores: [{ id: Date.now(), code: scoreTypes[0]?.code || '', title: scoreTypes[0]?.title || '', weight: 0 }] }
                    ];
                    newQuestions[realIdx] = { ...question, options: newOptions };
                    setAdminQuestions(newQuestions);
                  }}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, minHeight: 144, justifyContent: 'center' }}>
                  ...
                  {editModeId === question.id && (
                    <button
                      className={styles.adminEditButton}
                      aria-label="Вниз"
                      disabled={idx === adminQuestions.filter(q => q.isRequired === false).length - 1}
                      onClick={() => handleMoveQuestionDown(question.id)}
                      style={{ cursor: idx === adminQuestions.filter(q => q.isRequired === false).length - 1 ? 'not-allowed' : 'pointer', background: 'none', border: 'none', padding: 0 }}
                    >
                      <ArrowDownOutlinedSize24
                        initialColor={idx === adminQuestions.filter(q => q.isRequired === false).length - 1 ? 'secondary' : 'primary'}
                        backgroundStyle={'primary'}
                        borderRadius={12}
                        shadow="level-1"
                        padding={16}
